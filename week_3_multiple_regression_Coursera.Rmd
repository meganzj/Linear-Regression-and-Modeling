...
ggplot(evals, aes(x = score)) + 
         geom_histogram(binwidth=0.1, lwd=0.2 ) + 
         stat_bin(binwidth=0.1, geom="text", 
           aes(label=..count..), position=position_stack(vjust=0.5)) +
                          geom_density(alpha=0.2)


quantile(evals$score, prob = seq(0,1,0.25))

#summarise(evals$score)
```
...
print(summary(mod1))
print(plot(mod1$fitted, mod1$residual))
...
#plot(mod1$bty_avg, mod1$residual)
#qqline(mod1$residual)
ggplot(mod1, aes(x = evals$bty_avg, y = mod1$residual )) + geom_jitter()
ggplot(mod1, aes(sample=mod1$residual))+stat_qq()+theme_bw()
ggplot(mod1, aes(x = mod1$residual)) + 
         geom_histogram(binwidth=0.1, lwd=0.2 )
```
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_red1 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
...
#remove + cls_profs
m6 = lm(score ~ ethnicity + rank +  gender  + language + age + cls_perc_eval + 
...
#remove + rank
m8 = lm(score ~ ethnicity  +  gender  + language + age + cls_perc_eval + 
    cls_students + cls_level  + cls_profs + cls_credits+ bty_avg , data = evals)
